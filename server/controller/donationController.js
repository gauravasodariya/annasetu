...
async function getAllDonations(req, res) {
  try {
    const donations = await Donation.find().sort({ date: -1 }).populate('donor', 'name');
    res.json(donations);
  } catch (err) {
    console.error(err.message);
    res.status(500).send('Server Error');
  ...
  try {
    ...
    try {
      const notification = new Notification({
        type: 'NEW_DONATION',
        message: `New food donation available: ${req.body.foodType}`,
        relatedId: savedDonation._id,
        forRoles: ['admin', 'volunteer'],
      });
...
async function notifyDonationStatus(req, res) {
  try {
    const donation = await Donation.findById(req.params.id)
      .populate('donor', 'name')
    ...
    if (recipientType === 'donor') {
      ...
      if (!donation.assignedAgent) {
        return res.status(400).json({ msg: 'No agent assigned to this donation' });
      }
      recipient = donation.assignedAgent._id;
    } else {
      return res.status(400).json({ msg: 'Invalid recipient type' });
    }
    const notification = new Notification({
      user: recipient,
      title: `Donation Update: ${donation.foodType}`,
      message,
      type: 'donation',
...
async function getAvailableDonationsForNgo(req, res) {
  try {
    ...
    const donations = await Donation.find({ availability: 'available', status: 'COMPLETED' })
      .sort({ expiryDate: 1 })
      .populate('donor', 'name');
    res.json(donations);
  } catch (err) {
...
async function updateStatusAssignAgent(req, res) {
  try {
    ...
    const notification = new Notification({
      ...
      message: `Your donation of ${donation.foodType} has been ${status.toLowerCase()}`,
      type: 'donation',
      relatedId: donation._id,
    });
    await notification.save();
    if (assignedAgent) {
      const agentNotification = new Notification({
        ...
        message: 'A new donation has been assigned to you for pickup',
        type: 'donation',
        relatedId: donation._id,
      });
    ...
    const updatedDonation = await Donation.findById(donation._id)
      .populate('donor', 'name email phone')
      .populate('assignedAgent', 'name email phone');
    res.json({ donation: updatedDonation, msg: `Donation ${status.toLowerCase()} successfully` });
  } catch (err) {
...
async function updateDonation(req, res) {
  try {
    ...
    donation = await Donation.findByIdAndUpdate(req.params.id, { $set: updateData }, { new: true });
    res.json(donation);
  } catch (err) {
